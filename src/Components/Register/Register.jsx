...
async function sendRegisterData() {
 let{data}= await axios.post(`https://userapi-5mf0.onrender.com/user/sign-up`,user);
 console.log(data);
...
function ValidateRegisterForm() {
  let schema= Joi.object({
   userName:Joi.string().min(2).max(10).required().messages({
    ...
    "string.max":"Namelength must be less than or equal to 10 characters long"
   }),
   email: Joi.string()
   .email({ minDomainSegments: 2, tlds: { allow: ['com', 'net'] } }),
   password: Joi.string()
   .pattern(/^[A-Z][a-z]{8,10}/).messages({ 
    "string.pattern.base":"password must start with capital Letter and at least 8 or more characters"
   })
  });
                ...
                <form  className="text-center" onSubmit={submitRegisterForm}>
                  ...
                  <div className="mb-2 p-1">
                    ...
                    <input
                     onChange={getUserData}
                      ...
                      name="userName"
                    />
                    {errorList.filter((err)=> err.context.label=="userName")[0]? <p className="text-danger mb-0">{errorList.filter((err)=> err.context.label=="userName")[0]?.message}</p>:''}
                  </div>
                  <div className="mb-2 p-1">
                    {" "}
                    <input
                  ...
                  <div className="w-100">
                    <button className="btn w-75" type="submit">
                      {loading == true ? <i className="fas  fa-spinner fa-spin"></i>:'Sign Up'}
                      </button>
                  </div>
                </form>
              </div>
